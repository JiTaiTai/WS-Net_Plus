...
class IDWT_2D2(nn.Module):
    def __init__(self, wave):
        super(IDWT_2D2, self).__init__()
        w = pywt.Wavelet(wave)
        rec_hi = torch.Tensor(w.rec_hi)
        rec_lo = torch.Tensor(w.rec_lo)
        
        w_ll = rec_lo.unsqueeze(0)*rec_lo.unsqueeze(1)
        w_lh = rec_lo.unsqueeze(0)*rec_lo.unsqueeze(1)
        w_hl = rec_lo.unsqueeze(0)*rec_lo.unsqueeze(1)
        w_hh = rec_lo.unsqueeze(0)*rec_lo.unsqueeze(1)
    ...
    def forward(self, x):
        return IDWT_Function2.apply(x, self.filters)

class DWT_2D2(nn.Module):
    def __init__(self, wave):
        super(DWT_2D2, self).__init__()
        w = pywt.Wavelet(wave)
        dec_hi = torch.Tensor(w.dec_hi[::-1]) 
        dec_lo = torch.Tensor(w.dec_lo[::-1])

        w_ll = dec_lo.unsqueeze(0)*dec_lo.unsqueeze(1)
        w_lh = dec_lo.unsqueeze(0)*dec_lo.unsqueeze(1)
        w_hl = dec_lo.unsqueeze(0)*dec_lo.unsqueeze(1)
        w_hh = dec_lo.unsqueeze(0)*dec_lo.unsqueeze(1)
    ...
    def forward(self, x):
        return DWT_Function2.apply(x, self.w_ll, self.w_lh, self.w_hl, self.w_hh)


class DWT_Function3(Function):
    @staticmethod
    def forward(ctx, x, w_ll, w_lh, w_hl, w_hh):
        x = x.contiguous()
        ctx.save_for_backward(w_ll, w_lh, w_hl, w_hh)
        ...
        x_hh = torch.nn.functional.conv2d(x, w_hh.expand(dim, -1, -1, -1), stride = 2, groups = dim)
        x = torch.cat([x_ll, x_lh, x_hl, x_hh], dim=1)
        return x,x_ll,x_lh,x_hl,x_hh

    @staticmethod
    def backward(ctx, dx,w_ll,w_lh,w_hl,w_hh):
        # w_ll, w_lh, w_hl, w_hh = ctx.saved_tensors
        if ctx.needs_input_grad[0]:
            w_ll, w_lh, w_hl, w_hh = ctx.saved_tensors
            B, C, H, W = ctx.shape
        ...
        return dx, w_ll, w_lh, w_hl, w_hh

class IDWT_Function3(Function):
    @staticmethod
    def forward(ctx, x, filters):
        ctx.save_for_backward(filters)
        ctx.shape = x.shape
...
class IDWT_2D3(nn.Module):
    def __init__(self, wave):
        super(IDWT_2D3, self).__init__()
        w = pywt.Wavelet(wave)
        rec_hi = torch.Tensor(w.rec_hi)
        rec_lo = torch.Tensor(w.rec_lo)
    ...
    def forward(self, x):
        return IDWT_Function3.apply(x, self.filters)

class DWT_2D3(nn.Module):
    def __init__(self, wave):
        super(DWT_2D3, self).__init__()
        w = pywt.Wavelet(wave)
        dec_hi = torch.Tensor(w.dec_hi[::-1]) 
        dec_lo = torch.Tensor(w.dec_lo[::-1])
    ...
    def forward(self, x):
        # _,C,_,_ = x.size()
        return DWT_Function3.apply(x, self.w_ll, self.w_lh, self.w_hl, self.w_hh)
