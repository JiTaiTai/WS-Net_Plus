...
def train(opt):
    ...
    for epoch in range(opt.epochs):
        ...
        swa_ckp_path = os.path.join(swa_ckp_save_path, str(epoch + 1) + ".pt")  
        if epoch == 0:
            swa_model = model
        else:
            last_swa_ckp_path = os.path.join(swa_ckp_save_path, str(epoch) + ".pt")  
            swa_model = weights_swa(last_swa_ckp_path, last_ckp_path, epoch)  
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrain", type=str,
                        default="./runs/train/11/best_ckp/tf_efficientnetv2_s_in21k_w40_fpn+aspp+fuse+drop_fcn_epoch_183_0.88166.pt")  
    parser.add_argument("--cuda", type=str, default="2") 
    parser.add_argument("--dataset-dir", type=str, default="/zq2/dataset/CD-dataset/whu3/")  
