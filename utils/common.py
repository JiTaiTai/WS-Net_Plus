...
def get_metrics_offline(gt_dir, pred_dir, dual_label=True):
    ...
    for name in tqdm(names):
        label1 = cv2.imread(os.path.join(gt_dir, "label1", name))[..., 0]   
...
class SaveResult:
    ...
    def show(self, p, r, f1, miou, oa,
             refer_metric=np.array(0), best_metric=0, train_avg_loss=0, val_avg_loss=0, lr=0, epoch=0, exp_loss=0):
        print(
        ...
        with open(self.result_save_path, "a")as f:
            f.write(
                ('%-7s'+'%-12s' * 10) % (str(epoch), str(round(lr, 8)),
                                        ...
                                        str(round(exp_loss, 6))) + "\n")

        plot_results(self.result_save_path)

    def save_first_batch(self, batch_img1, batch_img2, batch_label1, batch_label2, name=0): 
